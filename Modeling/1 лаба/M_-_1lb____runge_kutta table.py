...
RK32 = []
met_rk(H1, RK11, RK12)
print('length with h1   ', len(RK11))
met_rk(H2, RK21, RK22)
print('length with h2   ', len(RK21))
met_rk(H3, RK31, RK32)
print('length with h3   ', len(RK31))
'''
EN1 = []
EN2 = []
met_el_n(H, EN1, EN2)
...
while i <= 600:
    ...
    print('rk1[i] = ', RK11[i])
    print('rk2[10i] = ', RK21[i*10])
    print('rk3[100i] = ', RK31[i*100])
    print('{:3.5f}  |{:10.3f}  | {:10.3f}  | {:10.3f}  |{:10.3f}'.format(x,RK11[i],RK21[i*10],RK31[i*100]))
    x += H1
    xo += H1
    i += 1
...
while x <= 3:
    #if (int(x) != int(xo)) or x == 0:
    print('{:3.5f}  |{:10.3f}  | {:10.3f}  | {:10.3f}  |{:10.3f}'.format(x,RK12[i],RK22[i*10],RK32[i*100]))
    x += H1
    xo += H1
    i += 1
